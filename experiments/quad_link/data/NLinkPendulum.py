# those can be installed using this https://github.com/LCAS/RKN/tree/master/n_link_sim
...
class NLinkPendulum:
    ...
    def __init__(self,
        ...
        self.init_vel_limits = init_vel_limits
        self.g = 9.81
    ...
    def _sample_sequences(self, num_seqs, seq_length):
        ...
        if self.init_vel_limits == 0:
            vel = [np.zeros([num_seqs, 1]) for _ in range(self.links)]
        else:
            vel = [self.random.uniform(self.init_vel_limits[0], self.init_vel_limits[1], (num_seqs, 1))
...
if __name__ == "__main__":
    n_link = NLinkPendulum(episode_length=75,
                           train_episodes=8000,
                           test_episodes=2000,
    ...
    np.savez_compressed("ql.npz",
                        train_obs=train_obs, train_targets=train_targets, test_obs=test_obs, test_targets=test_targets)
